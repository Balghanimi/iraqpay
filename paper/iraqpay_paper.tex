...
To demonstrate the benchmark suite and characterize gateway performance, we executed 50 payment creation requests per gateway against sandbox endpoints. Table~\ref{tab:benchmark-results} summarizes the results.

\begin{table}[!t]
\centering
\caption{Gateway benchmark results (50 iterations per gateway, sandbox mode)}
\label{tab:benchmark-results}
\small
\begin{tabular}{@{}lrrrrrrr@{}}
\toprule
\textbf{Gateway} & \textbf{N} & \textbf{Success\%} & \textbf{Avg (ms)} & \textbf{Med (ms)} & \textbf{P95 (ms)} & \textbf{P99 (ms)} & \textbf{StdDev} \\
\midrule
ZainCash & 50 & --- & --- & --- & --- & --- & --- \\
FIB      & 50 & --- & --- & --- & --- & --- & --- \\
QiCard   & 50 & --- & --- & --- & --- & --- & --- \\
...
\end{tabular}
\vspace{4pt}
\raggedright\small\textit{Note: Dashes (---) indicate values to be filled after running live sandbox benchmarks. COD is local-only (no network I/O).}
\end{table}
